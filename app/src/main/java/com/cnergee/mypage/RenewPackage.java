...
public class RenewPackage extends BaseActivity{
	...
	Dialog pg_dialog;
	AdditionalAmount additionalamt_obj;
	@Override
						...
						if(final_price_discount.length>1){
							...
							if(Double.valueOf(PackageRate)>0.0){
								if(is_renew_running){
									...
									// i = new Intent(RenewPackage.this,MakeMyPayment_PayTmActivity.class);

									Intent i = null;
									if(Utils.is_billdesk)
										i = new Intent(RenewPackage.this,MakeMyPayments_Billdesk.class);
...
//										i = new Intent(RenewPackage.this,MakeMyPayment_EBS.class);
	...
	public void parseAdditionalAmtData(String json){
		...
		try {
			...
			if(NewDataSetJson.has("AdditionAmount")){
				...
				additionalamt_obj= new AdditionalAmount(PackageRate,AdditionalAmount, AdditionalAmountType, DaysFineAmount, DiscountAmount, finalcharges, FineAmount, DiscountPercentage,is_cheque_bounce);
	...
	public void show_pg_dialog(String check) {
        ...
        RelativeLayout rl_payment_gateway_3=(RelativeLayout) pg_dialog.findViewById(R.id.rl_pg_3);
        ...
        if(CCAvenue>0){
            rl_payment_gateway_3.setVisibility(View.VISIBLE);
        }else{
            rl_payment_gateway_3.setVisibility(View.GONE);
        }
        ...
        rl_payment_gateway_3.setOnClickListener(new OnClickListener() {
	...
	public void show_payment_options(String check) {
        ...
        Log.e("Billdesk",":"+Billdesk);
     /*   Log.e("Atom",":"+Atom);
        Log.e("CCAvenue",":"+CCAvenue);
        */

        if(Atom>0&&CCAvenue>0&&EBS>0){
            Utils.is_atom=true;
            Utils.is_CCAvenue=true;
            Utils.is_ebs=true;
            show_pg_dialog(check);
        }else if(Atom>0&&CCAvenue>0){
            Utils.is_atom=true;
            Utils.is_CCAvenue=true;
            Utils.is_ebs=false;
            show_pg_dialog(check);
        }else if(CCAvenue>0&&EBS>0){
            Utils.is_atom=false;
            Utils.is_CCAvenue=true;
            Utils.is_ebs=true;
            show_pg_dialog(check);
        }else if(Atom>0&&EBS>0){
            Utils.is_atom=true;
            Utils.is_CCAvenue=false;
            Utils.is_ebs=true;
            show_pg_dialog(check);
